if __name__ == '__main__':
    ...
    #print('Test output sample:', test[:5])  # 打印测试输出的一部分



    print('Evaluating...')
    acc = evaluation(test, conf['data'])
    ##print('Evaluation result sample:', acc[:5])  # 打印评估结果的一部分

    # 检查 de_diag 函数
    print('Diag result NM:', de_diag(acc[0, :, :, 0], True))
    print('Diag result BG:', de_diag(acc[1, :, :, 0], True))
    print('Diag result CL:', de_diag(acc[2, :, :, 0], True))
    ...
    print("测试时间",datetime.now() - _time1)
    print("------------------%d"% opt.iter)
