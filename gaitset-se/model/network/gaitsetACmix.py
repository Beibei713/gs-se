...
class SetNetWithACmix(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        self.se1_block = SEWithAdvancedSpatialAttention(in_channels=_set_channels[0], reduction=16)
        self.se2_block = SEWithAdvancedSpatialAttention(in_channels=_gl_in_channels, reduction=16)

                # 归一化
        self.bn_fg = nn.BatchNorm2d(_set_channels[0])
        self.bn_x = nn.BatchNorm2d(_set_channels[0])
        self.bn_gl = nn.BatchNorm2d(_gl_in_channels)
    ...
    def forward(self, silho, batch_frame=None):
        ...
        gl = self.frame_max(x)[0]
        gl = self.se2_block(gl)
        gl = self.gl_layer1(gl)
        ...
        fg_middle = self.se1_block(fg_middle)
        ...
        fg_bottom = self.se1_block(fg_bottom)
